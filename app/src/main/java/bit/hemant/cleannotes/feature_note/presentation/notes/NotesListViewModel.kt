...
import bit.hemant.cleannotes.feature_note.domain.use_case.NotesUseCase
...
@HiltViewModel
class NotesListViewModel @Inject constructor(val useCase: NotesUseCase) : ViewModel() {
    ...
    init {
        getNotes(NoteOrder.Date(OrderType.Acending))
    }

    fun onEvent(notesEvent: NotesEvent) {
        when (notesEvent) {
            is NotesEvent.Order -> {
                if (state.value.noteOrder::class == notesEvent.noteOrder::class && state.value.noteOrder.orderType == notesEvent.noteOrder.orderType) {
                    return
                }
            ...
            is NotesEvent.DeleteNote -> {
                viewModelScope.launch {
                    useCase.deleteNoteUseCase.invoke(notesEvent.note)
                    recentlyDeletedNote = notesEvent.note
                }
            }
            is NotesEvent.RestoreNote -> {
                viewModelScope.launch {
                    useCase.addNoteUseCase(recentlyDeletedNote ?: return@launch)
    ...
    private fun getNotes(noteOrder: NoteOrder) {
        getnoteJob?.cancel()
        getnoteJob = useCase.getNotesUseCase(noteOrder).onEach {
